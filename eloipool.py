...
def newBlockNotificationSIGNAL(signum, frame):
    ...
    thr = threading.Thread(target=newBlockNotification,
                           name='newBlockNotification via signal %s' % (signum,))
    thr.daemon = True
    thr.start()
...
if __name__ == "__main__":
    ...
    # configure and initialize all loggers, add them to loggersShare list
    ...
    # Load and configure all authentication modules
    ...
    for i in config.Authentication:
        ...
        try:
            ...
            logging.getLogger('authentication').error(
                "Error setting up authentication module %s: %s", name, sys.exc_info())
