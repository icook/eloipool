...
def dblsha(b):
    """ Double sha hash, standard hash for bitcoin protocol """
...
class ScheduleDict:
    """ A priority queue to schedule multiple sockets in an async socket
    server """
    def __init__(self):
        self._dict = {}
        self._build_heap()
    ...
    def nextTime(self):
        """ Gets the next item off the priority queue """
        while True:
            (t, k, o) = self._heap[0]
            if k in self._dict:
                break
