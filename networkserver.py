...
class NetworkListener:
    ...
    def __init__(self, server, server_address, address_family=socket.AF_INET6):
        ...
        tryErr(self.setup_socket,
               server_address,
               Logger=self.logger,
               ErrorMsg=server_address)
...
class _Waker:
    def __init__(self, server, fd):
        ...
        self.logger = logging.getLogger(
            'Waker for %s' % (server.__class__.__name__,))
...
class AsyncSocketServer:
    ...
    waker = False
    # should we lock access to the priority queue? not needed if we're
    # not processing workers on the queue from multiple threads
    schMT = False
    ...
    def __init__(self, RequestHandlerClass):
        ...
        # error handlers for each scheduled coroutine
        self._schEH = {}
        # disable lock if requested
        if self.schMT:
    ...
    def wakeup(self):
        if not self.waker:
            raise NotImplementedError(
                'Class `%s\' did not enable waker' % (self.__class__.__name__))
