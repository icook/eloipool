...
class merkleMaker(threading.Thread):
    ...
    def __init__(self, *a, **k):
        ...
        # (block hash big endian hex, height, difficulty in hex)
    ...
    def _prepare(self):
        ...
        # Create a dictionary for fast lookups from uri or name
        URI2Name = {}
        Name2URI = {}
        ...
        def URINamePair(a, defname):
            """ Load the uri or name into a dictionary that has one or the
            other """
            if 'name' not in a:
        ...
        LeveledTS = {}
        # build a dictionary that includes priority information for each
        # of the sources
        ...
        # flatten the dictionary into a tuple in order of priority
        ...
        # add the service proxy to each block submission by resolving
        # the uri from available information
    ...
    def createClearMerkleTree(self, height):
        # Calculate the amount that should be recieved for generating a
        # new block of this height
    ...
    def _makeBlockSafe(self, MP, txnlist, txninfo):
        """ Does a bunch of checks to build a block after safety checks """
        # calculate blocksize by summing each raw transaction and adding header
        # size
    ...
    def _CallGBT(self, TS):
        """ Attempts to retrieve a block template from a template source
        (bitcoind client or similar) """
        access = TS['access']
        ...
        try:
            ...
            try:
                # Failing that, give BIP 22 draft (2012-02 through 2012-07)
                # getmemorypool a chance
            ...
            if not access.OldGMP:
                access.OldGMP = True
                self.logger.warning(
                    "Upstream '%s' is not BIP 22 compatible" % (TS['name'],))
    ...
    def _ProcessGBT(self, MP, TS=None):
        ...
        # reverse the previous hash into bytes
    ...
    def _updateMerkleTree_fromTS(self, TS):
        """ Attempt to update the merkel tree with new transactions we retrieve
        from a specific template server """
        MP = self._CallGBT(TS)
    ...
    def _updateMerkleTree(self):
        ...
        # if the update call fails, this sets the next retry time
        self.nextMerkleUpdate = now + self.TxnUpdateRetryWait

        # try to update our merkel tree from any of the sources we have listed
        self._updateMerkleTree_I()

        # successful update, set next update time
    ...
    def makeCoinbase(self, height):
        ...
        if len(rv) > 95:
            ...
            if self.overflowed < t - 300:
                self.logger.warning(
                    'Overflowing coinbase data! %d bytes long' % (len(rv),))
                self.overflowed = t
    ...
    def _doing(self, what):
        ...
        if self._doing_last:
            self.logger.debug(
                "Switching from (%4dx in %5.3f seconds) %s => %s"
                % (self._doing_i, now - self._doing_s, self._doing_last, what))
    ...
    def _makeOne(self, putf, merkleTree, height):
        ...
        # Only add it if the height hasn't changed in the meantime, to avoid a
        # race
    ...
    def makeClear(self):
        ...
        self._makeOne(self.clearMerkleRoots.put,
                      self.curClearMerkleTree,
                      height=self.currentBlock[1])

    def makeNext(self):
        self._doing('longpoll merkle roots')
        self._makeOne(self.nextMerkleRoots.put,
                      self.nextMerkleTree,
                      height=self.currentBlock[1] + 1)

    def makeRegular(self):
        self._doing('regular merkle roots')
        self._makeOne(self.merkleRoots.append,
                      self.currentMerkleTree,
                      height=self.currentBlock[1])
    ...
    def merkleMaker_II(self):
        ...
        # First, ensure we have the minimum clear, next, and regular (in that
        # order)
        ...
        # If we've met the minimum requirements, consider updating the merkle
        # tree
        ...
        if self.nextMerkleRoots.qsize() < self.WorkQueueSizeLongpoll[1]:
            return self.makeNext()
        if (len(self.merkleRoots) < self.WorkQueueSizeRegular[1] or
                self.merkleRoots[0][1] != self.currentMerkleTree):
            return self.makeRegular()

        # Nothing left to do, fire onBlockUpdate event (if appropriate) and
        # sleep
        if self.needMerkle == 1:
            self.onBlockUpdate()
            self.needMerkle = False
        self._doing('idle')
        # TODO: rather than sleepspin, block until MinimumTxnUpdateWait expires
    ...
    def merkleMaker_I(self):
        ...
        if (self.needMerkle == 1 and
                now > self.needMerkleSince + self.WarningDelayTxnLongpoll):
    ...
    def getMRD(self):
        try:
            ...
            self.LowestMerkleRoots = min(len(self.merkleRoots),
                                         self.LowestMerkleRoots)
            rollPrevBlk = False
        except IndexError:
            qsz = self.clearMerkleRoots.qsize()
            if qsz < 0x10:
                self.logger.warning(
                    'clearMerkleRoots running out! only %d left' % (qsz,))
            MRD = None
            while MRD is None:
                MRD = self.clearMerkleRoots.get()
            self.LowestClearMerkleRoots = min(self.clearMerkleRoots.qsize(),
                                              self.LowestClearMerkleRoots)
            rollPrevBlk = True
...
def _test():
    ...
    # setup a fake logger that can be turned on or off
    class fakelogger:
        LO = False
    ...
    txninfo = [{'fee': 0, 'sigops': 1},
               {'fee': 5, 'sigops': 10000},
               {'fee': 0, 'sigops': 10001}]

    def MBS(LO=False):
        m = deepcopy((MP, txnlist, txninfo))
        MM.logger.LO = LO  # turn logging on or off
        try:
            MM._makeBlockSafe(*m)
        except:
            if LO < 2:
